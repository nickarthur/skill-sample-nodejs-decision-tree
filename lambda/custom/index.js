...
let handlers = {
    ...
    'RecommendationIntent': function () {
        ...
        let key =
            `${slotValues.salaryImportance.resolved}-${slotValues.personality.resolved}-${slotValues.bloodTolerance.resolved}-${slotValues.preferredSpecies.resolved}`;
        let occupation = options[slotsToOptionsMap[key]];

        console.log(
            `${DEPLOYMENT_STAGE}: ` +
            'look up key: ',
            key, 'object: ', occupation);

        let speechOutput = 'So you want to be ' +
            slotValues.salaryImportance.resolved + '. You are an ' +
            slotValues.personality.resolved + ', you like ' +
            slotValues.preferredSpecies.resolved + '  and you ' +
    ...
    'SessionEndedRequest': function () {
        console.log(
            `${DEPLOYMENT_STAGE}: ` +
            'Session ended with reason: ' + this.event.request.reason);
    },
    ...
    'AMAZON.HelpIntent': function () {
        this.response
            ...
            .listen('Would you like a career or do you want to be a couch potato?');
        this.emit(':responseReady');
    },
    ...
    'Unhandled': function () {
        this.response.speak(
            'Sorry, I didn\'t get that. You can try: \'alexa, tell Decision Tree to' +
            ' recommend a job.\'');
    }
};
...
const REQUIRED_SLOTS = ['preferredSpecies', 'bloodTolerance', 'personality', 'salaryImportance'];
...
function getSlotValues(filledSlots) {
    ...
    Object.keys(filledSlots).forEach(function (item) {
        ...
        // console.log(`${DEPLOYMENT_STAGE}: ` + "item in filledSlots:
    ...
    // console.log(`${DEPLOYMENT_STAGE}: ` + "slot values:
...
function delegateSlotCollection() {
    console.log(
        `${DEPLOYMENT_STAGE}: ` +
        'in delegateSlotCollection');
    console.log(
        `${DEPLOYMENT_STAGE}: ` +
        'current dialogState: ' + this.event.request.dialogState);
    ...
    if (this.event.request.dialogState === 'STARTED') {
        ...
        console.log(
            `${DEPLOYMENT_STAGE}: ` +
            'disambiguated: ' + JSON.stringify(this.event));
        ...
        console.log(
            `${DEPLOYMENT_STAGE}: ` +
            'in completed');
        // console.log(`${DEPLOYMENT_STAGE}: ` + "returning: "+
        // JSON.stringify(this.event.request.intent));
...
function disambiguateSlot() {
    ...
    Object.keys(this.event.request.intent.slots).forEach(function (slotName) {
        ...
        if (currentSlot.confirmationStatus !== 'CONFIRMED' &&
            ...
            if (currentSlot.resolutions.resolutionsPerAuthority[0].status.code ===
                'ER_SUCCESS_MATCH') {
                ...
                if (currentSlot.resolutions.resolutionsPerAuthority[0].values.length >
                    1) {
                    ...
                    currentSlot.resolutions.resolutionsPerAuthority[0].values.forEach(
                        function (element, index, arr) {
                            prompt +=
                                ` ${(index === size - 1) ? " or" : " "} ${element.value.name}`;
                        });
                ...
                console.log(
                    `${DEPLOYMENT_STAGE}: ` +
                    'NO MATCH FOR: ',
                    currentSlot.name, ' value: ', currentSlot.value);
...
function slotHasValue(request, slotName) {
    ...
    // console.log(`${DEPLOYMENT_STAGE}: ` + "request =
